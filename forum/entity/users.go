...
type userR struct {
	AuthorArticles ArticleSlice `boil:"AuthorArticles" json:"AuthorArticles" toml:"AuthorArticles" yaml:"AuthorArticles"`
	...
	FollowingUsers UserSlice    `boil:"FollowingUsers" json:"FollowingUsers" toml:"FollowingUsers" yaml:"FollowingUsers"`
}
...
var (
	userAllColumns            = []string{"id", "created_at", "updated_at", "deleted_at", "username", "email", "password", "bio", "image"}
	userColumnsWithoutDefault = []string{"created_at", "updated_at", "deleted_at", "username", "email", "password", "bio", "image"}
...
// AuthorArticles retrieves all the article's Articles with an executor via author_id column.
func (o *User) AuthorArticles(mods ...qm.QueryMod) articleQuery {
...
func (o *User) Articles(mods ...qm.QueryMod) articleQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`favorites` on `articles`.`id` = `favorites`.`article_id`"),
...
func (o *User) FollowerUsers(mods ...qm.QueryMod) userQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`follows` on `users`.`id` = `follows`.`follower_id`"),
...
// FollowingUsers retrieves all the user's Users with an executor via id column.
func (o *User) FollowingUsers(mods ...qm.QueryMod) userQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`follows` on `users`.`id` = `follows`.`following_id`"),
...
// LoadAuthorArticles allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadAuthorArticles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	if singular {
		object.R.AuthorArticles = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.AuthorID) {
				local.R.AuthorArticles = append(local.R.AuthorArticles, foreign)
				if foreign.R == nil {
					foreign.R = &articleR{}
				}
...
func (userL) LoadArticles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("`articles`.id, `articles`.created_at, `articles`.updated_at, `articles`.deleted_at, `articles`.slug, `articles`.title, `articles`.description, `articles`.body, `articles`.author_id, `a`.`user_id`"),
		qm.From("`articles`"),
		qm.InnerJoin("`favorites` as `a` on `articles`.`id` = `a`.`article_id`"),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.CreatedAt, &one.UpdatedAt, &one.DeletedAt, &one.Slug, &one.Title, &one.Description, &one.Body, &one.AuthorID, &localJoinCol)
		if err != nil {
...
func (userL) LoadFollowerUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("`users`.id, `users`.created_at, `users`.updated_at, `users`.deleted_at, `users`.username, `users`.email, `users`.password, `users`.bio, `users`.image, `a`.`following_id`"),
		qm.From("`users`"),
		qm.InnerJoin("`follows` as `a` on `users`.`id` = `a`.`follower_id`"),
		qm.WhereIn("`a`.`following_id` in ?", args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		...
		for _, foreign := range resultSlice {
			...
			foreign.R.FollowingUsers = append(foreign.R.FollowingUsers, object)
		}
		return nil
	}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				...
				foreign.R.FollowingUsers = append(foreign.R.FollowingUsers, local)
				break
			}
		}
	}
...
// LoadFollowingUsers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadFollowingUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.Select("`users`.id, `users`.created_at, `users`.updated_at, `users`.deleted_at, `users`.username, `users`.email, `users`.password, `users`.bio, `users`.image, `a`.`follower_id`"),
		qm.From("`users`"),
		qm.InnerJoin("`follows` as `a` on `users`.`id` = `a`.`following_id`"),
	...
	if singular {
		object.R.FollowingUsers = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userR{}
			}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.FollowingUsers = append(local.R.FollowingUsers, foreign)
				if foreign.R == nil {
					foreign.R = &userR{}
				}
...
// AddAuthorArticles adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.AuthorArticles.
// Sets related.R.Author appropriately.
func (o *User) AddAuthorArticles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Article) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			AuthorArticles: related,
		}
	} else {
		o.R.AuthorArticles = append(o.R.AuthorArticles, related...)
	}
...
// SetAuthorArticles removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Author's AuthorArticles accordingly.
// Replaces o.R.AuthorArticles with related.
// Sets related.R.Author's AuthorArticles accordingly.
func (o *User) SetAuthorArticles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Article) error {
	...
	if o.R != nil {
		for _, rel := range o.R.AuthorArticles {
		...
		o.R.AuthorArticles = nil
	}
	return o.AddAuthorArticles(ctx, exec, insert, related...)
}

// RemoveAuthorArticles relationships from objects passed in.
// Removes related items from R.AuthorArticles (uses pointer comparison, removal does not keep order)
// Sets related.R.Author.
func (o *User) RemoveAuthorArticles(ctx context.Context, exec boil.ContextExecutor, related ...*Article) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.AuthorArticles {
			if rel != ri {
				continue
			}

			ln := len(o.R.AuthorArticles)
			if ln > 1 && i < ln-1 {
				o.R.AuthorArticles[i] = o.R.AuthorArticles[ln-1]
			}
			o.R.AuthorArticles = o.R.AuthorArticles[:ln-1]
			break
		}
	}
...
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Users's Articles accordingly.
// Replaces o.R.Articles with related.
// Sets related.R.Users's Articles accordingly.
func (o *User) SetArticles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Article) error {
	...
	removeArticlesFromUsersSlice(o, related)
	if o.R != nil {
		o.R.Articles = nil
	}
...
func (o *User) RemoveArticles(ctx context.Context, exec boil.ContextExecutor, related ...*Article) error {
	...
	var err error
	query := fmt.Sprintf(
		"delete from `favorites` where `user_id` = ? and `article_id` in (%s)",
...
func removeArticlesFromUsersSlice(o *User, related []*Article) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
// Sets related.R.FollowingUsers appropriately.
func (o *User) AddFollowerUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	...
	for _, rel := range related {
		query := "insert into `follows` (`following_id`, `follower_id`) values (?, ?)"
		values := []interface{}{o.ID, rel.ID}
	...
	for _, rel := range related {
		if rel.R == nil {
			rel.R = &userR{
				FollowingUsers: UserSlice{o},
			}
		} else {
			rel.R.FollowingUsers = append(rel.R.FollowingUsers, o)
		}
	}
	return nil
}
...
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.FollowingUsers's FollowerUsers accordingly.
// Replaces o.R.FollowerUsers with related.
// Sets related.R.FollowingUsers's FollowerUsers accordingly.
func (o *User) SetFollowerUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	query := "delete from `follows` where `following_id` = ?"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	removeFollowerUsersFromFollowingUsersSlice(o, related)
	if o.R != nil {
...
func (o *User) RemoveFollowerUsers(ctx context.Context, exec boil.ContextExecutor, related ...*User) error {
	...
	var err error
	query := fmt.Sprintf(
		"delete from `follows` where `following_id` = ? and `follower_id` in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.ID}
	for _, rel := range related {
...
func removeFollowerUsersFromFollowingUsersSlice(o *User, related []*User) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
		for i, ri := range rel.R.FollowingUsers {
			if o.ID != ri.ID {
				continue
			}

			ln := len(rel.R.FollowingUsers)
			if ln > 1 && i < ln-1 {
				rel.R.FollowingUsers[i] = rel.R.FollowingUsers[ln-1]
			}
			rel.R.FollowingUsers = rel.R.FollowingUsers[:ln-1]
			break
		}
	}
}

// AddFollowingUsers adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.FollowingUsers.
// Sets related.R.FollowerUsers appropriately.
func (o *User) AddFollowingUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	if o.R == nil {
		o.R = &userR{
			FollowingUsers: related,
		}
	} else {
		o.R.FollowingUsers = append(o.R.FollowingUsers, related...)
	}
...
// SetFollowingUsers removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.FollowerUsers's FollowingUsers accordingly.
// Replaces o.R.FollowingUsers with related.
// Sets related.R.FollowerUsers's FollowingUsers accordingly.
func (o *User) SetFollowingUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	query := "delete from `follows` where `follower_id` = ?"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	removeFollowingUsersFromFollowerUsersSlice(o, related)
	if o.R != nil {
		o.R.FollowingUsers = nil
	}
	return o.AddFollowingUsers(ctx, exec, insert, related...)
}

// RemoveFollowingUsers relationships from objects passed in.
// Removes related items from R.FollowingUsers (uses pointer comparison, removal does not keep order)
// Sets related.R.FollowerUsers.
func (o *User) RemoveFollowingUsers(ctx context.Context, exec boil.ContextExecutor, related ...*User) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from `follows` where `follower_id` = ? and `following_id` in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.ID}
	for _, rel := range related {
	...
	for _, rel := range related {
		for i, ri := range o.R.FollowingUsers {
			if rel != ri {
				continue
			}

			ln := len(o.R.FollowingUsers)
			if ln > 1 && i < ln-1 {
				o.R.FollowingUsers[i] = o.R.FollowingUsers[ln-1]
			}
			o.R.FollowingUsers = o.R.FollowingUsers[:ln-1]
			break
		}
	}
...
func removeFollowingUsersFromFollowerUsersSlice(o *User, related []*User) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
