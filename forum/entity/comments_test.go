...
func testCommentToOneArticleUsingArticle(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.ArticleID, foreign.ID)
	...
	if err = local.L.LoadArticle(ctx, tx, false, (*[]*Comment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCommentToOneSetOpArticleUsingArticle(t *testing.T) {
	var err error
	...
	for i, x := range []*Article{&b, &c} {
		...
		if !queries.Equal(a.ArticleID, x.ID) {
...
func testCommentToOneRemoveOpArticleUsingArticle(t *testing.T) {
	var err error
	...
	if err = a.SetArticle(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveArticle(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.Article().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if !queries.IsValuerNil(a.ArticleID) {
...
var (
	commentDBTypes = map[string]string{`ID`: `bigint`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`, `DeletedAt`: `datetime`, `ArticleID`: `bigint`, `UserID`: `bigint`, `Body`: `longtext`}
	_              = bytes.MinRead
)
