...
func testTagToManyArticles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into `article_tags` (`tag_id`, `article_id`) values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into `article_tags` (`tag_id`, `article_id`) values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := TagSlice{&a}
	if err = a.L.LoadArticles(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTagToManyAddOpArticles(t *testing.T) {
	var err error
...
func testTagToManySetOpArticles(t *testing.T) {
	var err error
...
func testTagToManyRemoveOpArticles(t *testing.T) {
	var err error
...
var (
	tagDBTypes = map[string]string{`ID`: `bigint`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`, `DeletedAt`: `datetime`, `Tag`: `longtext`}
	_          = bytes.MinRead
)
